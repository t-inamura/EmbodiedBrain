 ...
 * ＜その他＞
 * ・設定ファイル名は、config.ini
 * ・GUIの設計には、wxFormBuilderを使用している（SenseOfSelfGUI.hとSenseOfSelfGUI.cppはwxFormBuilderにより生成している）
 */
...
const std::string Param::PARAM_FILE_NAME = "SenseOfSelfGUI.ini";
...
bool MainApp::OnInit()
{
	...
	SenseOfSelfGUIMainFrame *frame = new SenseOfSelfGUIMainFrame(NULL, wxGetApp().argc, wxGetApp().argv.GetArguments());
