 ...
 * ＜その他＞
 * ・設定ファイル名は、config.ini
 * ・GUIの設計には、wxFormBuilderを使用している（ImitationMotionAccumulatorGUI.hとImitationMotionAccumulatorGUI.cppはwxFormBuilderにより生成している）
 */
...
const std::string Param::PARAM_FILE_NAME = "ImitationMotionAccumulatorGUI.ini";
...
bool MainApp::OnInit()
{
	...
	ImitationMotionAccumulatorGUIMainFrame *frame = new ImitationMotionAccumulatorGUIMainFrame(NULL, wxGetApp().argc, wxGetApp().argv.GetArguments());
