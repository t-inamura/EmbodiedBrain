...
{
	this->serviceName = serviceNameDefault;
	bool available = checkService(this->serviceName);
	if (available && this->kinectV2Service == NULL) {
		this->kinectV2Service = connectToService(this->serviceName);
	}
	else if (!available && this->kinectV2Service != NULL) {
		this->kinectV2Service = NULL;
	}
